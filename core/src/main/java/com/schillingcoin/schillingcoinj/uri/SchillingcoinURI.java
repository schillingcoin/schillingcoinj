...
/**
 * <p>Provides a standard implementation of a Schillingcoin URI with support for the following:</p>
 *
 * <ul>
 ...
 *
 * <ul>
 * <li>{@code schillingcoin:<address>}</li>
 * <li>{@code schillingcoin:<address>?<name1>=<value1>&<name2>=<value2>} with multiple
 ...
 */
public class SchillingcoinURI {
    /**
     * Provides logging for this class
     */
    private static final Logger log = LoggerFactory.getLogger(SchillingcoinURI.class);
    ...
    public static final String[] PEERCOIN_SCHEMES = {"schillingcoin"};
    public static final String PREFERED_PEERCOIN_SCHEME = "schillingcoin";
    ...
    /**
     * Constructs a new SchillingcoinURI from the given string. Can be for any network.
     *
     * @param uri The raw URI data to be parsed (see class comments for accepted formats)
     * @throws SchillingcoinURIParseException if the URI is not syntactically or semantically valid.
     */
    public SchillingcoinURI(String uri) throws SchillingcoinURIParseException {
        this(MainNetParams.get(), uri);
    }

    /**
     * Constructs a new object by trying to parse the input as a valid SchillingcoinURI object.
     *
     ...
     * @params prefix The coin prefix to check for or null for the schillingcoin
     * defaults
     *
     * @throws SchillingcoinURIParseException If the input fails Schillingcoin URI syntax and semantic checks.
     */
    public SchillingcoinURI(
        ...
        try {
            ...
            throw new SchillingcoinURIParseException("Bad URI syntax", e);
        }

        // URI is formed as  schillingcoin:<address>?<query parameters>
        // blockchain.info generates URIs of non-BIP compliant form schillingcoin://address?....
        // We support both until Ben fixes his code.

        // Remove the schillingcoin scheme.
        // (Note: getSchemeSpecificPart() is not used as it unescapes the label and parse then fails.
        // For instance with : schillingcoin:129mVqKUmJ9uwPxKJBnNdABbuaaNfho4Ha?amount=0.06&label=Tom%20%26%20Jerry
        ...
        if (prefix == null) {
            ...
            if (prefix == null) {
                throw new SchillingcoinURIParseException(
                    "Not a schillingcoin  URI scheme: " + uri.getScheme()
                );
            }
        ...
        if (addressSplitTokens.length == 0)
            throw new SchillingcoinURIParseException("No data found after the " + prefix + " prefix");
        ...
        if (!addressToken.isEmpty()) {
            // Attempt to parse the addressToken as a Schillingcoin address for this network
            try {
                Address address = new Address(params, addressToken);
                putWithValidation(FIELD_ADDRESS, address);
            } catch (final AddressFormatException e) {
                throw new SchillingcoinURIParseException("Bad address", e);
            }
        }

        if (addressToken.isEmpty() && getPaymentRequestUrl() == null) {
            throw new SchillingcoinURIParseException("No address and no r= parameter found");
        }
    }

    /**
     * Constructs a new object by trying to parse the input as a valid Schillingcoin URI.
     *
     ...
     * @throws SchillingcoinURIParseException If the input fails Schillingcoin URI syntax and semantic checks.
     */
    public SchillingcoinURI(NetworkParameters params, String input) throws SchillingcoinURIParseException {
     ...
     */
    private void parseParameters(@Nullable NetworkParameters params, String addressToken, String[] nameValuePairTokens) throws SchillingcoinURIParseException {
        // Attempt to decode the rest of the tokens into a parameter map.
        for (String nameValuePairToken : nameValuePairTokens) {
            final int sepIndex = nameValuePairToken.indexOf('=');
            if (sepIndex == -1)
                throw new SchillingcoinURIParseException("Malformed Schillingcoin URI - no separator in '" +
                        nameValuePairToken + "'");
            if (sepIndex == 0)
                throw new SchillingcoinURIParseException("Malformed Schillingcoin URI - empty name '" +
                        nameValuePairToken + "'");
     ...
     */
    private void putWithValidation(String key, Object value) throws SchillingcoinURIParseException {
        if (parameterMap.containsKey(key)) {
            throw new SchillingcoinURIParseException(String.format("'%s' is duplicated, URI is invalid", key));
        } else {
    ...
    /**
     * The Schillingcoin Address from the URI, if one was present. It's possible to have Schillingcoin URI's with no address if a
     ...
     *         10,000,000 units is 1 OES. May be null if no amount is specified
     */
    ...
    public String toString() {
        StringBuilder builder = new StringBuilder("SchillingcoinURI[");
        boolean first = true;
    ...
    public static String convertToSchillingcoinURI(Address address, Coin amount, String label, String message) {
        return convertToSchillingcoinURI(address.toString(), amount, label, message);
    }

    /**
     * Simple Schillingcoin URI builder using known good fields.
     *
     * @param address The Schillingcoin address
     * @param amount The amount
     * @param label A label
     * @param message A message
     * @return A String containing the Schillingcoin URI
     */
    public static String convertToSchillingcoinURI(
