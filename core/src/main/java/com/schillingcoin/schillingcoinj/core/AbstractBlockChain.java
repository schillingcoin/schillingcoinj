 ...
 * the lightweight SPV mode, a {@link com.schillingcoin.schillingcoinj.store.SPVBlockStore} is the right choice.</p>
 *
 ...
 * {@link com.schillingcoin.schillingcoinj.store.H2FullPrunedBlockStore} implements full verification, which is equivalent to the
...
public abstract class AbstractBlockChain {
     ...
     *
     * Following this one down to the genesis block produces the story of the economy from the creation of Schillingcoin
        ...
        for (Transaction tx : transactions) {
            try {
                if (listener.isTransactionRelevant(tx)) {
                    ...
                    if (clone)
                        tx = new Transaction(tx.params, tx.serialize());
