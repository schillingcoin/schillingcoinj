...
/**
 * <p>A Message is a data structure that can be serialized/deserialized using both the Schillingcoin proprietary serialization
...
public abstract class Message implements Serializable {
     ...
     * @param protocolVersion Schillingcoin protocol version.
    ...
    // These methods handle the serialization/deserialization using the custom Schillingcoin protocol.
    ...
    /**
     * Should only used by SchillingcoinSerializer for cached checksum
     *
     * @return the checksum
     */
    byte[] getChecksum() {
    ...
    /**
     * Should only used by SchillingcoinSerializer for caching checksum
     *
    ...
    /**
     * Returns a copy of the array returned by {@link Message#unsafeSerialize()}, which is safe to mutate.
    ...
    /**
     * Serialize this message to a byte array that conforms to the schillingcoin wire protocol.
     * <br/>
    ...
    public byte[] unsafeSerialize() {
        ...
        if (parseRetain) {
            ...
            // which means another call to serialize is coming.  If we didn't recache then internal
    ...
    /**
     * Serialize this message to the provided OutputStream using the schillingcoin wire format.
     *
     * @param stream
     * @throws IOException
     */
    final public void serialize(OutputStream stream) throws IOException {
    ...
    /**
     * Serializes this message to the provided stream. If you just want the raw payload use serialize().
     */
    void serializeToStream(OutputStream stream) throws IOException {
        log.error("Error: {} class has not implemented serializeToStream method.  Generating message with no payload", getClass());
    }

    /**
     * This method is a NOP for all classes except Block and Transaction.  It is only declared in Message
     * so SchillingcoinSerializer can avoid 2 instanceof checks + a casting.
     */
