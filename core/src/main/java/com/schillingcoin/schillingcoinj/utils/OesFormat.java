...
import com.schillingcoin.schillingcoinj.utils.OesAutoFormat.Style;
import static com.schillingcoin.schillingcoinj.utils.OesAutoFormat.Style.*;
 ...
 *       {@link OesFormat#format(Object)} method.
 *   <li>Parse a value by passing a <code>String</code>-type
 *       representation of it to the {@link OesFormat#parse(String)} method.</ol>
 *
 * <p>For example, depending on your locale, values might be formatted
 * and parsed as follows:
 *
 * <blockquote><pre>
 * OesFormat f = OesFormat.getInstance();
 * String c = f.format(Coin.COIN);                <strong>// "OES 1.00"</strong>
 * String k = f.format(Coin.COIN.multiply(1000)); <strong>// "OES 1,000.00"</strong>
 * String m = f.format(Coin.COIN.divide(1000));   <strong>// "mOES 1.00"</strong>
 * Coin all = f.parseObject("MS 21");             <strong>// All the money in the world</strong>
 * </pre></blockquote>
 *
 ...
 * be formatted, {@link OesAutoFormat}, and another that formats any value in units of a
 * fixed, specified denomination, {@link OesFixedFormat}.
 *
 ...
 * values of {@link OesAutoFormat.Style}.  There are two styles constants: {@link
 * OesAutoFormat.Style#CODE} (the default), and {@link OesAutoFormat.Style#SYMBOL}.  The
 * difference is that the <code>CODE</code> style uses an internationally-distinct currency
 * code, such as <code>"OES"</code>, to indicate the units of denomination, while the
 ...
 * locale, a value of one peercoin might be formatted as <pre>S1.00</pre> where a value
 * exceeding that by one satoshi would be <pre>µS1,000,000.01</pre>
 *
 ...
 * <code>1.0000 OES</code>, or one peercoin, in millipeercoins, one shifts the decimal point
 * three places, that is, to <code>1000.0 mOES</code>.
 *
 * <h3>Construction</h3>
 *
 * There are two ways to obtain an instance of this class: <ol>
 *   <li>Use one of the factory methods; or
 *   <li>Use a {@link OesFormat.Builder} object.</ol>
 *
 ...
 * methods of this abstract base class <code>OesFormat</code>.  There
 ...
 * automatic-denominating <code>OesAutoFormat</code> instance for your
 ...
 * <blockquote><pre>
 * OesFormat f = OesFormat.getInstance();
 * String s = f.format(Coin.COIN); <strong>// "OES 1.00"</strong>
 * </pre></blockquote>
 *
 ...
 * the {@link OesFixedFormat} instance that is returned, and thus will
 * determine its denomination.  For example, if you want to format
 * values in units of micropeercoins:
 *
 * <blockquote><pre>OesFormat m = OesFormat.getInstance(6);
 ...
 *
 * <blockquote><pre>OesFormat milliFormat = OesFormat.getInstance(MILLICOIN_SCALE);</pre></blockquote>
 *
 * <p>Alternatively, if the type of the first argument to
 * <code>getInstance()</code> is one of the enumerated values of the
 * {@link OesAutoFormat.Style} type, either <code>CODE</code> or
 * <code>SYMBOL</code>, then you will get a {@link OesAutoFormat}
 * instance that uses either a currency code or symbol, respectively,
 * to indicate the results of its auto-denomination.
 *
 * <blockquote><pre>
 * OesFormat s = OesFormat.getInstance(SYMBOL);
 * Coin value = Coin.parseCoin("0.1234");
 * String mil = s.format(value);              <strong>// "₥S123.40"</strong>
 * String mic = s.format(value.divide(1000)); <strong>// "µS123.40"</strong>
 * </blockquote>
 *
 ...
 *
 * <blockquote><pre>
 * <strong>// Next line returns "1,00 OES"</strong>
 * OesFormat.getInstance(Locale.GERMANY).format(Coin.COIN);
 * <strong>// Next line returns "1,00 S"</strong>
 * OesFormat.getInstance(SYMBOL, Locale.GERMANY).format(Coin.COIN);
 * <strong>// Next line returns "1.000,00"</strong>
 * OesFormat.getMilliInstance(Locale.GERMANY).format(Coin.COIN);
 * <strong>// Next line returns "10.000,00"</strong>
 * OesFormat.getInstance(4, Locale.GERMANY).format(Coin.COIN);
 * </pre></blockquote>
 *
 ...
 *
 * A new {@link OesFormat.Builder} instance is returned by the {@link #builder()} method.  Such
 * an object has methods that set the configuration parameters of a <code>OesFormat</code>
 * object.  Its {@link Builder#build()} method constructs and returns a <code>OesFormat</code> instance
 ...
 *
 * <blockquote><pre>
 * OesFormat.Builder builder = OesFormat.builder();
 * builder.style(SYMBOL);
 * builder.symbol("B&#x5c;u20e6"); <strong>// unicode char "double vertical stroke overlay"</strong>
 * OesFormat f = builder.build();
 * String out = f.format(COIN); <strong>// "S1.00" depending on locale</strong>
 * </pre></blockquote>
 *
 ...
 *
 * <blockquote><pre>
 * OesFormat f = OesFormat.builder().style(CODE).code("OES").build();
 * String out = f.format(COIN); <strong>// "OES 1.00"</strong>
 * </pre></blockquote>
 *
 * <p>See the documentation of the {@link OesFormat.Builder} class for details.
 *
 * <h3>Formatting</h3>
 *
 * <p>You format a Peercoin monetary value by passing it to the {@link OesFormat#format(Object)}
 * method.  This argument can be either a {@link com.schillingcoin.schillingcoinj.core.Coin}-type object or a
 ...
 * {@link OesFixedFormat} also accept a variable-length sequence of additional <code>int</code>
 ...
 *
 * <blockquote><pre>
 * OesFormat f = OesFormat.getCoinInstance();
 ...
 * {@link OesFixedFormat#REPEATING_PLACES}, {@link
 * OesFixedFormat#REPEATING_DOUBLETS} and {@link
 * OesFixedFormat#REPEATING_TRIPLETS}.  These signify repeating groups
 ...
 *
 * <blockquote><pre>
 * OesFormat de = OesFormat.getInstance(Locale.GERMANY);
 * FieldPosition currField = new FieldPosition(NumberFormat.Field.CURRENCY);
 * <strong>// next line formats the value as "987.654.321,23 µOES"</strong>
 * String output = de.format(valueOf(98765432123L), new StringBuffer(), currField);
 * <strong>// next line sets variable currencyCode to "µOES"</strong>
 ...
 *
 * <blockquote><pre>
 * OesFixedFormat kilo = (OesFixedFormat)OesFormat(-3); <strong>// scale -3 for kilocoins</strong>
 * Coin value = Coin.parseCoin("1230");
 * <strong>// variable coded will be set to "kOES 1.23"</strong>
 * String coded = kilo.code() + " " + kilo.format(value);
 * <strong>// variable symbolic will be set to "kS1.23"</strong>
 * String symbolic = kilo.symbol() + kilo.format(value);
 * OesFormat(4).code(); <strong>// unnamed denomination has no code; raises exception</strong>
 * </pre></blockquote>
 *
 ...
 *                SATOSHI};
 * OesFormat f = OesFormat.getCoinInstance(2, REPEATING_PLACES);
 ...
 * OesFixedFormat} and {@link OesAutoFormat} is analogous to the difference in formatting
 * behavior between instances of those classes.  Instances of {@link OesAutoFormat} recognize
 ...
 * instances of {@link OesFixedFormat} by default recognize no codes nor symbols, but rather
 ...
 * OesFixedFormat} can be overridden by setting a parsing pattern that includes a currency sign
 * using the {@link OesFormat.Builder#pattern()} method.
 *
 * <p>The {@link OesAutoFormat#parse(String)}</code> method of {@link OesAutoFormat} (and of
 * {@link OesAutoFormat} configured with applicable non-default pattern) will recognize a
 ...
 * specified by <code>µS</code>, <code>uS</code>, <code>µS</code>, <code>µS</code>,
 * <code>µOES</code> or other appropriate permutations of those characters.  Additionally, if
 * either or both of a custom currency code or symbol is configured using {@link
 * OesFormat.Builder#code()} or {@link OesFormat.Builder.code()}, then such code or symbol will
 ...
 * <code>OesFormat</code> instance, on the other hand, whether or not the units indicator must
 * be separated by a space from the number is determined by the locale.  The {@link
 * OesFormat#pattern()} method returns a representation of the pattern that
 * can be examined to determine whether a space must separate currency signs from numbers in
 * parsed <code>String</code>s.
 *
 * <p>When parsing, if the currency-units indicator is absent, then a {@link OesAutoFormat}
 * instance will infer a denomination of peercoins while a {@link OesFixedFormat} will infer the
 ...
 * @see          com.schillingcoin.schillingcoinj.core.Coin
 */

public abstract class OesFormat extends Format {
    ...
    /** The conventional international currency code for peercoins: "OES" */
    private static final String COIN_CODE = "OES";
    /** The default currency symbols for peercoins */
    private static final String COIN_SYMBOL = "S";
    /** An alternative currency symbol to use in locales where the default symbol is used for the national currency. */
    protected static final String COIN_SYMBOL_ALT = "S";
    ...
    /**
     * <p>This class constructs new instances of {@link OesFormat}, allowing for the
     * configuration of those instances before they are constructed.  After obtaining a
     * <code>Builder</code> object from the {@link OesFormat#builder()} method, invoke the
     * necessary setter methods to obtain your desired configuration.  Finaly, the {@link
     * #build()} method returns a new <code>OesFormat</code> object that has the specified
     * configuration.
     *
     * <p>All the setter methods override defaults.  Invoking <code>build()</code> without invoking any
     * of the setting methods is equivalent to invoking {@link OesFormat#getInstance()} with no arguments.
     *
    ...
    public static class Builder {
        ...
        private enum Variant {
            AUTO { @Override OesFormat newInstance(Builder b) {
                       return getInstance(b.style, b.locale, b.minimumFractionDigits);
                   }},
            FIXED,
            UNSET;
            OesFormat newInstance(Builder b) {
        ...
        private Style style = OesAutoFormat.Style.CODE;
        private int scale = 0;
        private String symbol = "",code = "",pattern = "",localizedPattern = "";

        private Builder() {}

        /** Specify the new <code>OesFormat</code> is to be automatically-denominating.
         * The argument determines which of either codes or symbols the new <code>OesFormat</code>
        ...
        public Builder style(OesAutoFormat.Style val) {
         ...
         * appropriately for the context of generating {@link OesAutoFormat} instances.
         *
         ...
         *
         * <p>The <code>OesFormat</code> class provides appropriately named
         * <code>int</code>-type constants for the three common values, {@link OesFormat#COIN_SCALE},
         * {@link OesFormat#MILLICOIN_SCALE} {@link OesFormat#MICROCOIN_SCALE}.
         *
         * <p>If neither this method nor {@link #style(OesAutoFormat.Style)} is invoked on a
         * <code>Builder</code>, then the <code>OesFormat</code> will default to a
         ...
         *  class.
         *
         *  <p>The pattern is localized according to the locale of the <code>OesFormat</code>
         *  instance, the symbols for which can be examined by inspecting the {@link
         *  java.text.DecimalFormatSymbols} object returned by {@link OesFormat#symbols()}.
        ...
        /** Return a new {@link OesFormat} instance.  The object returned will be configured according
         *  to the state of this <code>Builder</code> instance at the time this method is invoked. */
        public OesFormat build() {
            OesFormat f = variant.newInstance(this);
    ...
    protected OesFormat(DecimalFormat numberFormat, int minDecimals, List<Integer> groups) {
     ...
     * currency code, for example "<code>OES</code>", to indicate that denomination.  The
     ...
     */
    public static OesFormat getInstance() { return getInstance(defaultLocale()); }

    /**
     * Return a new auto-denominating instance that will indicate units using a currency
     * symbol, for example, <code>"S"</code>.  Formatting and parsing will be done
     * according to the default locale.
     */
    public static OesFormat getSymbolInstance() { return getSymbolInstance(defaultLocale()); }

    /**
     * Return a new auto-denominating instance that will indicate units using a currency
     * code, for example, <code>"OES"</code>.  Formatting and parsing will be done
     * according to the default locale.
     */
    public static OesFormat getCodeInstance() { return getCodeInstance(defaultLocale()); }
     ...
     * <code>"S"</code>.  The returned object will format the fraction-part of numbers using
     ...
     * locale.
     */
    public static OesFormat getSymbolInstance(int fractionPlaces) {
     ...
     * <code>"OES"</code>.  The returned object will format the fraction-part of numbers using
     * the given number of decimal places, or fewer as necessary to avoid giving a place to
     * fractional satoshis.  Formatting and parsing will be done according to the default
     * locale.
     */
    public static OesFormat getCodeInstance(int minDecimals) {
     ...
     * units using a currency code, for example, <code>"mOES"</code>.
     */
    public static OesFormat getInstance(Locale locale) { return getCodeInstance(locale); }
     ...
     * units using a currency code, for example, <code>"mOES"</code>.
     */
    public static OesFormat getCodeInstance(Locale locale) { return getInstance(CODE, locale); }
     ...
     * <code>"mOES"</code>.  The returned object will format the fraction-part of numbers using
     * the given number of decimal places, or fewer as necessary to avoid giving a place to
     * fractional satoshis.
     */
    public static OesFormat getInstance(Locale locale, int minDecimals) {
     ...
     * fractional satoshis.
     */
    public static OesFormat getCodeInstance(Locale locale, int minDecimals) {
     ...
     * select denominational units based on each value being formatted, and will indicate those
     * units using a currency symbol, for example, <code>"µS"</code>.
     */
    public static OesFormat getSymbolInstance(Locale locale) {
     ...
     * <code>"µS"</code>.  The returned object will format the fraction-part of numbers using
     * the given number of decimal places, or fewer as necessary to avoid giving a place to
     * fractional satoshis.
     */
    public static OesFormat getSymbolInstance(Locale locale, int fractionPlaces) {
     ...
     * <code>"S"</code>, or code, such as <code>"mOES"</code>, depending on the value of
     * the argument.  Formatting and parsing will be done according to the default locale.
     */
    public static OesFormat getInstance(Style style) { return getInstance(style, defaultLocale()); }
     ...
     * using either a currency symbol, such as, <code>"S"</code>, or code, such as
     * <code>"mOES"</code>, depending on the value of the first argument.  The returned object
     ...
     */
    public static OesFormat getInstance(Style style, int fractionPlaces) {
     ...
     * "<code>OES</code>", or symbol, such as "<code>S</code>", depending on the value
     ...
     */
    public static OesFormat getInstance(Style style, Locale locale) {
     ...
     * such as <code>"mOES"</code>, or symbol, such as "<code>S</code>",
     ...
     * fractional satoshis.
     */
    public static OesFormat getInstance(Style style, Locale locale, int fractionPlaces) {
	return new OesAutoFormat(locale, style, fractionPlaces);
    }
     ...
     */
    public static OesFormat getCoinInstance() { return getCoinInstance(defaultLocale()); }
     ...
     */
    public static OesFormat getCoinInstance(int minFractionPlaces, int... groups) {
     ...
     */
    public static OesFormat getCoinInstance(Locale locale) {
     ...
     */
    public static OesFormat getCoinInstance(Locale locale, int scale, int... groups) {
     ...
     */
    public static OesFormat getMilliInstance() { return getMilliInstance(defaultLocale()); }
     ...
     * necessary.
     */
    public static OesFormat getMilliInstance(Locale locale) {
     ...
     */
    public static OesFormat getMilliInstance(int scale, int... groups) {
     ...
     */
    public static OesFormat getMilliInstance(Locale locale, int scale, int... groups) {
     ...
     * necessary.
     */
    public static OesFormat getMicroInstance() { return getMicroInstance(defaultLocale()); }
     ...
     * necessary.
     */
    public static OesFormat getMicroInstance(Locale locale) {
     ...
     * necessary to avoid rounding, to a limiting precision of satoshis.
     */
    public static OesFormat getMicroInstance(int scale, int... groups) {
     ...
     * limiting precision of satoshis.
     */
    public static OesFormat getMicroInstance(Locale locale, int scale, int... groups) {
     ...
     * limiting precision of satoshis.
     */
    public static OesFormat getInstance(int scale, int minDecimals, int... groups) {
     ...
     */
    public static OesFormat getInstance(int scale) {
     ...
     */
    public static OesFormat getInstance(int scale, Locale locale) {
     ...
     */
    public static OesFormat getInstance(int scale, Locale locale, int minDecimals, int... groups) {
     ...
     */
    public static OesFormat getInstance(int scale, Locale locale, int minDecimals, List<Integer> groups) {
        return new OesFixedFormat(locale, scale, minDecimals, groups);
    }
    ...
    /**
      ...
      * {@link com.schillingcoin.schillingcoinj.core.Coin} object that represents the parsed value.
      * @see java.text.NumberFormat */
    @Override
    ...
    private volatile String ci = "(" + COIN_SYMBOL + "|" + COIN_SYMBOL_ALT + "|S|" + COIN_CODE + "|OES)";
     ...
     * for instances of {@link OesAutoFormat} and false for instances of {@link
     * OesFixedFormat}, then denominated (i.e., prefixed) currency signs in the parsed String
     ...
     * OesAutoFormat} will interpret the parsed number as a quantity of peercoins, and instances
     * of {@link OesAutoFormat} will interpret the number as a quantity of that instance's
     * configured denomination, which can be ascertained by invoking the {@link
     * OesFixedFormat#symbol()} or {@link OesFixedFormat#code()} method.
     *
    ...
    @Override public boolean equals(Object o) {
        ...
        if (!(o instanceof OesFormat)) return false;
        OesFormat other = (OesFormat)o;
