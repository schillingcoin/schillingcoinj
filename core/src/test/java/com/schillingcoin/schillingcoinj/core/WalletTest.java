...
import com.schillingcoin.schillingcoinj.signers.StatelessTransactionSigner;
...
import com.schillingcoin.schillingcoinj.testing.*;
...
import static com.schillingcoin.schillingcoinj.core.Utils.HEX;
import static com.schillingcoin.schillingcoinj.testing.FakeTxBuilder.*;
...
public class WalletTest extends TestWithWallet {
    ...
    // For schillingcoin we shall send two cents, one cent for fee and another to meet minimum output requirement
    ...
    private void basicSpendingCommon(Wallet wallet, Address toAddress, Address destination, boolean testEncryption) throws Exception {
        ...
        // will attach a small fee. Because the schillingcoin protocol makes it difficult to determine the fee of an
        ...
        try {
            ...
            assertEquals(valueOf(9, 1), e.missing); // Schillingcoin: Miss nine coins and cent fee
        }
    ...
    public void balance() throws Exception {
        ...
        spend.params = MainNetParams.get(); // Schillingcoin: Use normal selectable logic
        wallet.commitTx(spend);
        assertEquals(1, wallet.getPoolSize(WalletTransaction.Pool.PENDING));

        // Available and estimated balances should not be the same. We don't check the exact available balance here
        // because it depends on the coin selection algorithm.
        assertEquals(valueOf(4, 49), wallet.getBalance(Wallet.BalanceType.ESTIMATED)); // Schillingcoin: minus fee
    ...
    public void balances() throws Exception {
        ...
        Transaction send1 = wallet.createSend(new ECKey().toAddress(params), valueOf(0, 10));
        // Reserialize.
        Transaction send2 = new Transaction(params, send1.serialize());
        assertEquals(nanos, send2.getValueSentFromMe(wallet));
        assertEquals(ZERO.subtract(valueOf(0, 11)), send2.getValue(wallet)); // Schillingcoin: Cent fee
    }
    ...
    public void isConsistent_duplicates() throws Exception {
        ...
        Transaction txClone = new Transaction(params, tx.serialize());
        try {
    ...
    public void bounce() throws Exception {
        ...
        inbound2.addOutput(new TransactionOutput(params, inbound2, coinHalf.subtract(CENT), myAddress)); // Schillingcoin: Less a cent
        ...
        assertEquals(coin1.subtract(TWO_CENTS), wallet.getBalance()); // Schillingcoin: Two cents loss in two transactions
    }
    ...
    public void doubleSpendUnspendsOtherInputs() throws Exception {
        ...
        // Schillingcoin: Make sure main net is used to make coins unselectable
        ...
        send2 = new Transaction(params, send2.serialize());
        ...
        assertEquals(valueOf(1, 99), wallet.getBalance()); // Schillingcoin: minus fee
        assertTrue(wallet.isConsistent());
    }
    ...
    public void doubleSpends() throws Exception {
        ...
        buf[47] = 0;  // Break the signature: schillingcoinj won't check in SPV mode and this is easier than other mutations
        // Schillingcoin: we used 47 thus adding 4 for the timestamp
        send1 = new Transaction(params, buf);
        wallet.commitTx(send2);
        wallet.allowSpendingUnconfirmedTransactions();
        assertEquals(value.subtract(CENT), wallet.getBalance(Wallet.BalanceType.ESTIMATED)); // Schillingcoin: minus cent fee
        // Now spend the change. This transaction should die permanently when the mutant appears in the chain.
        Transaction send3 = checkNotNull(wallet.createSend(address, value.subtract(TWO_CENTS))); // Schillingcoin: minus the first fee and then another required fee
    ...
    public void pending1() throws Exception {
        ...
        final Transaction t1Copy = new Transaction(params, t1.serialize());
    ...
    public void spendToSameWallet() throws Exception {
        ...
        // Schillingcoin: We will use MainNetParams so to make pending transaction unselectable
    	outbound1.params = MainNetParams.get();
        wallet.commitTx(outbound1);
        // We should have a zero available balance before the next block.
        assertEquals(ZERO, wallet.getBalance());
        sendMoneyToWallet(outbound1, AbstractBlockChain.NewBlockType.BEST_CHAIN);
        // We should have a balance of 1 OES after the block is received.
        assertEquals(coin1.subtract(CENT), wallet.getBalance()); // Schillingcoin: minus cent fee
    }
    ...
    public void pubkeyOnlyScripts() throws Exception {
        ...
        Transaction t2 = wallet.createSend(new ECKey().toAddress(params), value.subtract(CENT)); // Schillingcoin: minus fee
        assertNotNull(t2);
    ...
    public void marriedKeychainBloomFilter() throws Exception {
        ...
        assertFalse(wallet.getBloomFilter(0.001).contains(outPoint.serialize()));

        wallet.receiveFromBlock(t1, b1, BlockChain.NewBlockType.BEST_CHAIN, 0);
        assertTrue(wallet.getBloomFilter(0.001).contains(outPoint.serialize()));
    }

    @Test
    public void autosaveImmediate() throws Exception {
        // Test that the wallet will save itself automatically when it changes.
        File f = File.createTempFile("schillingcoinj-unit-test", null);
    ...
    public void ageMattersDuringSelection() throws Exception {
        ...
        for (int i = 0; i < ITERATIONS; i++) {
            txns[i] = sendMoneyToWallet(COIN.add(CENT), AbstractBlockChain.NewBlockType.BEST_CHAIN); // Schillingcoin: Give room for fee
        }
    ...
    public void feeSolverAndCoinSelectionTest() throws Exception {
        ...
        // Spend it 
        SendRequest req = SendRequest.to(notMyAddr, CENT.multiply(11)); // Schillingcoin: minus cent fee
        ...
        // Schillingcoin: Check minimum fee is enforced
        ...
        // Schillingcoin: Check less than cent change is disposed of.
        ...
        SendRequest request6 = SendRequest.to(notMyAddr, Coin.COIN.subtract(TWO_CENTS)); // Schillingcoin: Extra cent for fee
        ...
        // We optimize for priority, so the output selected should be the largest one
        assertEquals(Coin.COIN.subtract(CENT), spend6.getOutput(0).getValue().add(spend6.getOutput(1).getValue())); // Schillingcoin: minus cent fee

        // Schillingcoin: All balance minus fee gives no change
        ...
        // Schillingcoin: Used 1.00 coin, now 0.9 due to fee
        ...
        assertEquals(CENT.add(SATOSHI), request15.tx.getFee()); // Schillingcoin: Should add kb fee on top of required fee per extra kb
        ...
        // Schillingcoin: Check less than 1kb transaction
        ...
        assertEquals(outValue19, COIN.add(CENT)); // Schillingcoin: We used two cents remember?
    ...
    public void basicCategoryStepTest() throws Exception {
        ...
        SendRequest request1 = SendRequest.to(notMyAddr, balance.subtract(CENT).subtract(SATOSHI)); // Schillingcoin: minus fee
        wallet.completeTx(request1);
        assertEquals(SATOSHI.add(CENT), request1.tx.getFee()); // Schillingcoin: cent fee added
    ...
    public void lowerThanDefaultFee() throws InsufficientMoneyException {
    	// Schillingcoin: Minimum fee forced
    ...
    public void feePerKbCategoryJumpTest() throws Exception {
        ...
        // Schillingcoin: With inputs of Coin plus cent we can pay 1.00 with no change
        ...
        // Schillingcoin: We can pay 1.001 with coin and two cents but change is discarded
        ...
        // Schillingcoin: We can pay 0.991 with coin and one cent but change is discarded so two cents are used instead to allow for change
    ...
    public void testCompleteTxWithExistingInputs() throws Exception {
        ...
        assertEquals(COIN.subtract(CENT.multiply(2)), request1.tx.getOutput(1).getValue()); // Schillingcoin: cent fee
    ...
    public void testEmptyWallet() throws Exception {
        ...
        Transaction tx = createFakeTx(params, TWO_CENTS, myAddress); // Schillingcoin: Need an extra cent for fee
        wallet.receiveFromBlock(tx, block, AbstractBlockChain.NewBlockType.BEST_CHAIN, 0);
    ...
    public void keyRotationRandom() throws Exception {
        ...
        assertEquals(CENT, tx.getOutput(0).getValue()); // Schillingcoin: Two cents minus fee to one cent remaining.
    ...
    public void completeTxPartiallySignedWithDummySigs() throws Exception {
        byte[] dummySig = TransactionSignature.dummy().encodeToSchillingcoin();
    ...
    public void sendRequestMemo() throws Exception {
        receiveATransaction(wallet, myAddress);
        SendRequest sendRequest = SendRequest.to(myAddress, Coin.COIN.subtract(CENT)); // Schillingcoin: minus fee
