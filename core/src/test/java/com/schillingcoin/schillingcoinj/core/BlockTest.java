...
public class BlockTest {
    ...
    public void testHeaderParse() throws Exception {
        ...
        Block reparsed = new Block(params, header.serialize());
    ...
    public void testSerialization() throws Exception {
        ...
        //
        // NB: This tests the Schillingcoin proprietary serialization protocol. A different test checks Java serialization
        // of transactions.
        Block block = new Block(params, blockBytes);
        assertArrayEquals(blockBytes, block.serialize());
    }
    ...
    public void testJavaSerialiazation() throws Exception {
        ...
        // Note that this will actually check the transactions are equal by doing schillingcoin serialization and checking
    ...
    public void testUpdateLength() {
        ...
        assertEquals(block.serialize().length, block.length);
        final int origBlockLen = block.length;
        Transaction tx = new Transaction(params);
        // this is broken until the transaction has > 1 input + output (which is required anyway...)
        //assertTrue(tx.length == tx.serialize().length && tx.length == 8);
        ...
        int origTxLength = 8 + 2 + 8 + 1 + 10 + 40 + 1 + 1 + 4; //Schillingcoin: 4 for timestamp
        assertEquals(tx.serialize().length, tx.length);
        ...
        assertEquals(block.length, block.serialize().length);
        assertEquals(block.length, origBlockLen + tx.length);
        assertEquals(tx.length, origTxLength - 1);
