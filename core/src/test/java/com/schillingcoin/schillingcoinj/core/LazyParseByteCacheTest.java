...
import static com.schillingcoin.schillingcoinj.testing.FakeTxBuilder.createFakeBlock;
import static com.schillingcoin.schillingcoinj.testing.FakeTxBuilder.createFakeTx;
...
public class LazyParseByteCacheTest {
    ...
    public void setUp() throws Exception {
        ...
        SchillingcoinSerializer bs = new SchillingcoinSerializer(unitTestParams);
    ...
    public void testBlock(byte[] blockBytes, boolean isChild, boolean lazy, boolean retain) throws Exception {
        ...
        //message structure.
        SchillingcoinSerializer bsRef = new SchillingcoinSerializer(unitTestParams, false, false);
        ByteArrayOutputStream bos = new ByteArrayOutputStream();

        SchillingcoinSerializer bs = new SchillingcoinSerializer(unitTestParams, lazy, retain);
        Block b1;
        Block bRef;
        b1 = (Block) bs.deserialize(ByteBuffer.wrap(blockBytes));
        bRef = (Block) bsRef.deserialize(ByteBuffer.wrap(blockBytes));

        //verify our reference SchillingcoinSerializer produces matching byte array.
        bos.reset();
    ...
    public void testTransaction(NetworkParameters params, byte[] txBytes, boolean isChild, boolean lazy, boolean retain) throws Exception {
        ...
        //reference serializer to produce comparison serialization output after changes to
        //message structure.
        SchillingcoinSerializer bsRef = new SchillingcoinSerializer(params, false, false);
        ByteArrayOutputStream bos = new ByteArrayOutputStream();

        SchillingcoinSerializer bs = new SchillingcoinSerializer(params, lazy, retain);
        Transaction t1;
        Transaction tRef;
    ...
    private void serDeser(SchillingcoinSerializer bs, Message message, byte[] sourceBytes, byte[] containedBytes, byte[] containingBytes) throws Exception {
        ByteArrayOutputStream bos = new ByteArrayOutputStream();
